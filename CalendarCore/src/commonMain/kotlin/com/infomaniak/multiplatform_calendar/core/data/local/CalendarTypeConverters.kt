...
internal class CalendarTypeConverters {
    ...
    @ColumnTypeConverter
    fun fromOrganizer(value: OrganizerEntity?): String? = value?.let { Json.encodeToString(it) }

    @ColumnTypeConverter
    fun toOrganizer(value: String?): OrganizerEntity? = value?.let { Json.decodeFromString(it) }
    ...
    @ColumnTypeConverter
    fun fromIcalDateValues(value: List<IcalDateValue>): String = Json.encodeToString(value)

    @ColumnTypeConverter
    fun toIcalDateValues(value: String): List<IcalDateValue> = Json.decodeFromString(value)
