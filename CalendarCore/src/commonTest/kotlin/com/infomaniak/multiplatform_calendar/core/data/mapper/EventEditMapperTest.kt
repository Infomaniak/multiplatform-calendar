...
class EventEditMapperTest {
    ...
    fun winterEvent_emitsWinterOffset_notSummerOffset() {
        ...
        val edit = editData(
            timing = EventTiming(
                start = EventDateTime.of(LocalDateTime(2026, 2, 15, 14, 0), paris),
                end = EventDateTime.of(LocalDateTime(2026, 2, 15, 15, 0), paris),
                isAllDay = false,
            ),
        ).toRemoteEdit(previous = null, stamp = STAMP)
    ...
    fun exDateCarryingAForeignTzid_keepsTheOccurrenceItDesignated() {
        ...
        // already designated, not reinterpret its wall clock in the event's own zone.
