...
class EventRepositoryTest : RobolectricTestsBase() {
    ...
    fun updateEvent_thisAndFollowing_atAnOccurrenceTheRuleDoesNotGenerate_isRejected() = runTest {
        ...
        assertFailsWith<IllegalArgumentException> {
            repository.updateEvent(
                credentials = DavAccount(baseUrl = "https://cal/", username = "u", password = "p"),
                occurrenceId = occurrenceOf(master.id, LocalDateTime(2026, 6, 16, 15, 0)),
                data = editData(
                    title = "Tail",
                    calendarId = calendarId,
                    recurrence = RecurrenceRule(freq = Frequency.Daily, occurrenceCount = 5),
    ...
    private fun recurringColorMaster(
        ...
        return EventEntity(
            ...
            rDates = rDates,
            hasRecurrence = true,
            recurrenceBounds = checkNotNull(toRecurrenceBoundsEntity(timing = timing, recurrenceRule = rrule, rDates = rDates)),
            etag = "1",
        )
    }
